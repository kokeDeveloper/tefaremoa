...
export async function GET(req: Request) {
  ...
  const publicOrigin = (process.env.NEXT_PUBLIC_BASE_URL ?? url.origin).replace(/\/$/, '')
  ...
  try {
    if (!email && !ssoToken) {
      const redirect = new URL('/admin/login', publicOrigin)
    ...
    const res = NextResponse.redirect(new URL(from, publicOrigin))
